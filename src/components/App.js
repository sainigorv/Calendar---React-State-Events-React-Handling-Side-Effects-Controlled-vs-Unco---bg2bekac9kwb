...
export default function App() {
  ...
  for (let i = 0; i < gapsToBeLeft(); i++) {
    ...
    totalCells.push(
      <td className="border border-secondary" key={cellNumber} id={id}>
        {""}
      </td>
    ),
      cellNumber++;
  ...
  for (let i = 1; i <= daysInMonth(); i++) {
    ...
    if (i === selectedCurrentDate())
      totalCells.push(
        <td
          className="border border-secondary bg-info"
          key={cellNumber}
          id={"today"}
        >
          {i}
  ...
  for (
    let i = Number(gapsToBeLeft()) + Number(daysInMonth());
    i % 7 !== 0;
    i++
  ) {
    const id = "cell" + cellNumber;
  ...
  totalCells.forEach((cell, i) => {
    if (i % 7 !== 0) {
      ...
      completeTable.push(
        <tr className="border border-secondary" key={i * 109 + 2}>
          {currRow}
        </tr>
      );
      oneRow = [];
        ...
        <div className="mx-auto">
          ...
          <div className="d-flex justify-content-around">
            <select
              ...
              onChange={(e) => {
                setMonth(moment.months().indexOf(e.target.value));
              }}
            >
            ...
            {showYearInput ? (
              ...
              <input
                id="year-text-box"
                defaultValue={selectedDate.format("Y")}
                onKeyUp={(e) => {
                  if (e.which === 13) {
          ...
          <div className="w-75 mx-auto">
            <button
              className="w-25"
              id="previous-year"
              onClick={() => alterYear(Number(selectedDate.format("Y")) - 1)}
            >
              {"<<"}
            </button>
            <button
              className="w-25"
              id="previous-month"
              onClick={() => setMonth(Number(selectedDate.format("M")) - 2)}
            >
              {"<"}
            </button>
            <button
              className="w-25"
              id="next-month"
              onClick={() => setMonth(Number(selectedDate.format("M")))}
            >
              {">"}
            </button>
            <button
